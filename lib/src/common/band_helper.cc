...
{
  uint32_t freq_MHz = (uint32_t)round(freq / 1e6);
  std::cout << "freq_MHz: " << freq_MHz << std::endl;
  // std::cout << freq_MHz << std::endl;
  for (const nr_operating_band& band : nr_operating_bands_fr1) {
    std::cout << "h and l: " << band.F_DL_low << "  " << band.F_DL_high << std::endl;
    if (freq_MHz >= band.F_DL_low and freq_MHz <= band.F_DL_high) {
...
std::vector<srsran_subcarrier_spacing_t> srsran_band_helper::get_all_ssb_scs(uint16_t band) const
{
  std::vector<srsran_subcarrier_spacing_t> res_scs;
  // Look for the given band and SCS
  for (const nr_band_ss_raster& ss_raster : nr_band_ss_raster_table) {
    // Check if band and SCS match!
    if (ss_raster.band == band) {
      res_scs.push_back(ss_raster.scs);
    }
...
double srsran_band_helper::sync_raster_t::get_frequency() const
{
  ...
  if (gscn_last <= 7498) {
    ...
    //   // std::cout << "freq: " << N * 1200e3 - 50e3 << std::endl;
    //   return N * 1200e3 - 50e3;
    // }
    // std::cout << "freq: " << N * 1200e3 + M[M_idx] * 50e3 << std::endl;
...
{
  // std::cout << "Input band: " << band << " Input scs: " << scs << std::endl;
  // Look for the given band and SCS
  for (const nr_band_ss_raster& ss_raster : nr_band_ss_raster_table) {
    // std::cout << "Valid band: " << ss_raster.band << " Valid scs: " << ss_raster.scs << std::endl;
    // Check if band and SCS match!
    if (ss_raster.band == band && ss_raster.scs == scs) {
      // std::cout << ss_raster.gscn_first << std::endl;
      // std::cout << ss_raster.gscn_step << std::endl;
      // std::cout << ss_raster.gscn_last << std::endl;

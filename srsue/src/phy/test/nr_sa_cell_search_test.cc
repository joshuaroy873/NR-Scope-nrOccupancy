...
struct args_t {
  ...
  void set_ssb_from_band()
  {
    ...
    // band = 78;
    std::cout << "Selected band: " << band << std::endl;

    srsran_assert(band != UINT16_MAX, "Invalid band");

    std::cout << "base_carrier.nof_prb: " << base_carrier.nof_prb << std::endl;
    std::cout << "base_carrier.dl_center_freq: " << base_carrier.dl_center_frequency_hz << std::endl;
...
int parse_args(int argc, char** argv, args_t& args)
{
 ...
 options.add(over_the_air).add(simulation).add(phy).add(stack).add_options()
     ...
     ("nof_prb", bpo::value<uint32_t>(&args.base_carrier.nof_prb)->default_value(args.base_carrier.nof_prb), "Number of prbs for the cell")
     ;
  // clang-format on
  ...
  std::cout << "dl freq: " << args.base_carrier.dl_center_frequency_hz << std::endl;
  std::cout << "nof prb: " << args.base_carrier.nof_prb << std::endl;
...
static cell_search_result_t cell_search(const args_t& args, dummy_ue& ue)
{
  ...
  uint32_t ssb_scs_hz             = srsran_subcarrier_spacing_30kHz;//SRSRAN_SUBC_SPACING_NR(args.ssb_scs);
  ...
  while (not ss.end()) {
    ...
    std::cout << "searching frequency: " << cs_args.ssb_freq_hz / 1e6 << " MHz" << std::endl;
    std::cout << "offset_hz: " << offset_hz << " Hz" << std::endl;
...
int main(int argc, char** argv)
{
  ...
  std::cout << "Center freq: " << args.base_carrier.dl_center_frequency_hz/1e6 << 
    "MHz. Is the cell found? " << found_cell.found << std::endl;

...
{
  // We need to modify most of this function's operation to suit our design 
...
int srsran_dmrs_pdcch_estimate_nrscope(srsran_dmrs_pdcch_estimator_t* q,
                               const srsran_slot_cfg_t*       slot_cfg,
                               const cf_t*                    sf_symbols)
{
  if (q == NULL || sf_symbols == NULL) {
  ...
  for (uint32_t l = 0; l < q->coreset.duration; l++) {
    ...
    // Extract pilots least square estimates
    srsran_dmrs_pdcch_extract(q, cinit, 
      &sf_symbols[l * q->carrier.nof_prb * SRSRAN_NRE], q->lse[l]);
  }

...
typedef struct {
  ...
  uint32_t starting_symbol;
} prach_nr_config_t;

// typedef struct {
//   // char preamble_format[10];
//   uint32_t x;
//   uint32_t y;
//   uint32_t subframe_number[PRACH_NR_CFG_MAX_NOF_SF];
//   uint32_t nof_subframe_number;
//   uint32_t starting_symbol;
// } prach_nr_config_t_extended_nrscope;
...
// it only includes the first 28 row entry of the table (with preamble format = 0).

...
bool cell_search::start(const cfg_t& cfg)
{
  ...
  // std::cout << "cell_search center freq: " << ssb_cfg.center_freq_hz << std::endl;
  ssb_cfg.ssb_freq_hz      = cfg.ssb_freq_hz;
  // std::cout << "cell_search ssb freq: " << cfg.ssb_freq_hz << std::endl;
  ...
  if (srsran_ssb_set_cfg(&ssb, &ssb_cfg) < SRSRAN_SUCCESS) {
    // std::cout << "Return false in cell search.cc" << std::endl;
...
cell_search::ret_t cell_search::run_slot(const cf_t* buffer, uint32_t slot_sz)
{
  ...
  // printf("center freq hz: %f\n", ssb.cfg.center_freq_hz);
  // if(ssb.cfg.center_freq_hz == 2.52375e+09){
  //   FILE *fp2;
  //   fp2 = fopen("found_nodata.txt", "a");
  //   fwrite(buffer, sizeof(cf_t), slot_sz, fp2);
  //   fclose(fp2);
  // }
  // std::cout << "slot size: " << slot_sz << std::endl;
  ...
  // std::cout << "cell_search.cc snr: " << ret.ssb_res.measurements.snr_dB << " "
  //  << ret.ssb_res.pbch_msg.crc << std::endl;
  return ret;
}

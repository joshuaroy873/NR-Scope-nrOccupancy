...
const prach_nr_config_t* srsran_prach_nr_get_cfg_fr1_unpaired(uint32_t config_idx)
{
  ...
  }else if(config_idx < PRACH_NR_CFG_FR1_UNPAIRED_NOF_CFG_NRSCOPE + PRACH_NR_CFG_FR1_UNPAIRED_NOF_CFG){
    return &prach_nr_cfg_fr1_unpaired_nrscope[config_idx - 
            PRACH_NR_CFG_FR1_UNPAIRED_NOF_CFG];
  }
...
{
  int ret = SRSRAN_ERROR;
  if (p != NULL && N_ifft_ul < 2049 && cfg->config_idx < 263 && cfg->root_seq_idx < MAX_ROOTS) {

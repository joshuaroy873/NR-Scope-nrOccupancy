                                  ...
                                  float noise_estimate)
{
  for (int i = 0; i < nof_symbols; i++) {
    ...
    cf_t hh = 0;
    for (int p = 0; p < nof_rxant; p++) {
      // if(cimag(h[p][i]) * cimag(y[p][i]) < 0){
      //   h[p][i] = creal(h[p][i]) - I*cimag(h[p][i]);
      // }
      // if(creal(h[p][i]) * creal(y[p][i]) < 0){
      //   h[p][i] = -creal(h[p][i]) + I*cimag(h[p][i]);
      // }
      r += y[p][i] * conjf(h[p][i]);
      hh += conjf(h[p][i]) * h[p][i];
                              ...
                              float  noise_estimate)
{
  ...
  // printf("precoding function\n");
  if (csi) {
    // printf("CSI precodeing\n");

...
static int ofdm_init_nr_nrscope(srsran_ofdm_t* q, srsran_ofdm_cfg_t* cfg, srsran_dft_dir_t dir, int scs_idx)
{
  // If the symbol size is not given, calculate in function of the number of resource blocks
  if (cfg->symbol_sz == 0) {
    int symbol_sz_err = srsran_symbol_sz(cfg->nof_prb);
...
{
  return ofdm_init_nr_nrscope(q, cfg, SRSRAN_DFT_FORWARD, scs_idx);
}

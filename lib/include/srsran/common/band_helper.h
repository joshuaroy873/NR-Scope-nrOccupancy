...
public:
  ...
  /**
   * @brief Select all the SSB subcarrier spacing valid for this band
   * @param band NR band number
   * @return The SSB subcarrier spacing
   */
  std::vector<srsran_subcarrier_spacing_t> get_all_ssb_scs(uint16_t band) const;
  ...
  protected:
      ...
      gscn_first(gscn_f), gscn_step(gscn_s), gscn_last(gscn_l), gscn(gscn_f)
    {
      if (gscn_last <= 7498) {
        ...
        N_last  = 2499;
        // changed by Haoran
        // N_first = (gscn_first - 1) / 3;
        // N_last = (gscn_last + 1) / 3;
      } else if (7499 <= gscn_last and gscn_last <= 22255) {
        N_last = 14756;
        // changed by Haoran
        // N_first = gscn_first - 7499;
        // N_last = gscn_last - 7499;
      } else if (22256 <= gscn_last and gscn_last <= 26639) {
        N_last = 4383;
        // changed by Haoran
        // N_first = gscn_first - 22256;
        // N_last = gscn_last - 22256;
      }
      N = N_first;
    }
    uint32_t gscn;

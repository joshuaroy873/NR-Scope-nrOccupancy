...
static const coreset_zero_t_f_entry_nrscope coreset_table_13_11[16] = {
    {0, 1, 1, 0},
    {0, 2, 1/2, 0}, // if i is even, first_symbol_idx = 0, else  N_symb_CORESET
    {2, 1, 1, 0},
    {2, 2, 1/2, 0},   // if i is even, first_symbol_idx = 0, else  N_symb_CORESET
    {5, 1, 1, 0},
    {5, 2, 1/2, 0},   // if i is even, first_symbol_idx = 0, else  N_symb_CORESET
    {7, 1, 1, 0},
    {7, 2, 1/2, 0},   // if i is even, first_symbol_idx = 0, else  N_symb_CORESET
    {0, 1, 2, 0},
    {5, 1, 2, 0},
    {0, 1, 1, 1},
    {0, 1, 1, 2},
...
int coreset_zero_t_f_nrscope(uint32_t entry_idx, uint32_t ssb_idx, uint32_t coreset_symb, coreset_zero_t_f_entry_nrscope* cfg){
  if(entry_idx > 15 || ssb_idx > 7 || coreset_symb > 3){
    ERROR("Invalid table entry input\n");
    return SRSRAN_ERROR;
  }
  // since we are using FR1 for the USRP
  cfg->O = coreset_table_13_11[entry_idx].O;
  cfg->M = coreset_table_13_11[entry_idx].M;
  cfg->nof_ss = coreset_table_13_11[entry_idx].nof_ss;
  if(entry_idx == 1 || entry_idx == 3 || entry_idx == 5 || entry_idx == 7){
    if (ssb_idx % 2 == 1){
      cfg->first_symbol_idx = coreset_symb;
    }
  }
  return SRSRAN_SUCCESS;
}
...
// Most of bands with 15/30 scs have the min_BW 5 or 10 MHz, except 96, 102 and 104 have min_BW 20 MHz.
// No need to expand now.
                        ...
                        srsran_coreset_t*           coreset)
{
  ...
  // printf("subc spacing nr: %d\n", SRSRAN_SUBC_SPACING_NR(ssb_scs));
  // printf("ssb half bw hz: %d\n", ssb_half_bw_Hz);
  // printf("ssb_pointA_freq_offset_Hz: %d\n", ssb_pointA_freq_offset_Hz);
